...
export default function CalculatorPage() {
  ...
  // weeklyChange is always in current unit (lbs or kg)
  const [weeklyChange, setWeeklyChange] = useState(0);

  // Convert input weight/height to metric for calculation
  const getWeightKg = () =>
  ...
  // Imperial: lbs/week * 3500 / 7
  // Metric:   kg/week  * 7700 / 7
  ...
  // Slider props for each unit
  const sliderProps =
    unit === "imperial"
      ? { min: -3, max: 3, step: 0.1 }
      : { min: -1.36, max: 1.36, step: 0.05 }; // ±3 lbs ≈ ±1.36 kg

  // When switching units, don't convert slider value—just let it reset to 0 (or remember last per-unit value)
  const handleUnitChange = (newUnit) => {
    if (unit === newUnit) return;
    setUnit(newUnit);
    setWeeklyChange(0);
    // Optional: convert height/weight fields for user
    if (newUnit === "imperial") {
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen p-6">
      ...
      <Link
        href="/dashboard"
        className="mt-8 px-4 py-2 bg-gray-600 text-white rounded-xl"
      >
        ← Back to Dashboard
      </Link>
    </main>
